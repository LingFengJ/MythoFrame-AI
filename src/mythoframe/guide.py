...
Use Dreamina/CapCut first, Runway as the reliable fallback, and seedance2.ai only as a cheap unofficial candidate after testing.
...
mythoframe import-asset {slug} video_clip <seedance-video> --shot 1 --candidate-id shot_001_clip_a --provider "Dreamina Seedance 2.0"
...
- Video/audio-video provider: Seedance 2.0 through Dreamina/CapCut first, used for image-to-video/text-to-video and native clip audio where possible.
- Local ffmpeg: local rough-cut assembly.

## Access Rule

Prefer Dreamina/CapCut web UI first. Use Runway as the reliable paid fallback. Treat seedance2.ai as a cheap unofficial candidate only after payment, export, and quality checks. Treat direct ByteDance/BytePlus/Volcengine API access as an optional later backend only after the user explicitly chooses API automation and accepts the provider cost/onboarding requirements.
